...
public class PasswordValidator implements ConstraintValidator<Password, String> {
    ...
    private static final String CONTAIN_SPECIAL_SYMBOL_ERROR_MESSAGE =
            "The password must contain one of these special symbols (!, @, #, $, %, ^, &, *, (, ), _, +, <, >, ?).";
    ...
    public boolean isValid(String value, ConstraintValidatorContext context) {
        ...
        if(!isContainingSpecialSymbol(value) && this.containsSpecialSymbol){
            return false;
        }
        return true;
    }
    ...
    private boolean isContainingSpecialSymbol(String password) {
        List<String> specialSymbol =
                Arrays.asList("!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "_", "+", "<", ">", "?");
        for (String character : specialSymbol) {
